...
export function registerInboxTools(server: McpServer, client: HimalayaClient) {
  ...
  server.registerTool("search_emails", {
    description: "Search emails using himalaya filter syntax. Multi-word phrases must be backslash-escaped (`subject quarterly\\ report`). Examples: 'subject invoice', 'from paypal', 'subject quarterly\\ report and from boss'. Operators: and, or, not. Conditions: subject, from, to, body, date, before, after, flag.",
    inputSchema: {
      query: z.string().describe("Search query in himalaya filter syntax. Multi-word values must be backslash-escaped (e.g. 'subject quarterly\\ report'). Single-word: 'subject invoice', 'from alice and subject meeting'. Use 'body <word>' to search email body text."),
